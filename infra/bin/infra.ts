...
const repositoryName = app.node.tryGetContext("repositoryName") || "PawRush/code-gpt-docs";
...
if (!codeConnectionArn) {
  ...
  new FrontendStack(app, `CodeGPTFrontend-${environment}`, {
    env: { account, region },
    environment,
    buildOutputPath,
    description: `CodeGPT Docs static website - ${environment}`,
...
if (codeConnectionArn) {
  new PipelineStack(app, "CodeGPTPipelineStack", {
    env: { account, region },
    description: "CI/CD Pipeline for CodeGPT",
    codeConnectionArn,
    repositoryName,
    branchName,
...
cdk.Tags.of(app).add("Project", "CodeGPT");
